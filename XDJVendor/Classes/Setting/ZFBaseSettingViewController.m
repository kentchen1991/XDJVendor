...
//    __block ZFSettingCell *weakCell = cell;
    __weak typeof(ZFSettingCell) *weakSelf = cell;
    cell.switchChangeBlock = ^ (BOOL on){
        if (weakSelf.item.switchBlock) {
            weakSelf.item.switchBlock(on);
        }
    };
...
-(void)setTableViewFootView:(UIView*)view {
    [_tableView setTableFooterView:view];
}

-(void)zfTableViewReload {
